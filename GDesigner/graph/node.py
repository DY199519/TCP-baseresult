...
class Node(ABC):
    ...
    def clear_connections(self):
        # Clear spatial connections (bidirectional)
        for predecessor in list(self.spatial_predecessors):
            if self in predecessor.spatial_successors:
                predecessor.spatial_successors.remove(self)
        for successor in list(self.spatial_successors):
            if self in successor.spatial_predecessors:
                successor.spatial_predecessors.remove(self)
        
        # Clear temporal connections (bidirectional)
        for predecessor in list(self.temporal_predecessors):
            if self in predecessor.temporal_successors:
                predecessor.temporal_successors.remove(self)
        for successor in list(self.temporal_successors):
            if self in successor.temporal_predecessors:
                successor.temporal_predecessors.remove(self)
